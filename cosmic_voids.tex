...
\begin{figure*}
  ...
  web scheme (T-Web is up. V-Web is down).  
...
voids.
For this purpose, we
use the \textit{watershed transform algorithm}
\citep{Beucher79,Beucher93} to identify a void as the basin of FA
local minimum. 
The advantage of this definition is that we can
now forget about the free parameter $\lambda_{th}$. 
Additionally, this algorithm does not require any assumption on the
shape and/or morphology of the tentative voids. 


The watershed technique has been already implemented to find voids \citet{Platen07,
Neyrinck08} however it used the density field instead of the FA
field as we suggest in this paper. 
Another difference with respect to other void finding implementations
is that we estimate all relevant quantities on a Cartesian mesh of fixed cell
size, while it other works use an adaptive Delaunay tessellation
\citep{Schaap00}. 
However from the analysis of our results this different does not seem
to introduce supurious results, at least with the mass resolution we
have in the N-body simulation.

This watershed algorithm still needs a threshold value to reduce
spurious features and to avoid void hierarchization.  
...
The right column in Figure \ref{fig:FA_field} shows all the voids that
have been identified using the watershed algorithm on the FA field.
