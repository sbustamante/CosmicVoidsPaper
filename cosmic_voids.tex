...
\begin{figure*}
  ...
  \caption{Different fields in a slice of
    ...
    in the right panels correspond with the effective radius of each void. }
  \label{fig:FA_field}
\end{figure*}
%.........................................................................
...
\centering
  ...
  width=0.30\textheight]{delta_L1.pdf}
  \includegraphics[trim = 1mm 0mm 5mm 10mm, clip, keepaspectratio=true,
  width=0.30\textheight]{FA_L1.pdf}  
  \includegraphics[trim = 1mm 0mm 5mm 10mm, clip, keepaspectratio=true,
  width=0.30\textheight]{FA_delta.pdf}

  \captionof{figure}{\small Density contrast (top) and Fractional
    Anisotropy (middle) with respect to the eigenvalue $\lambda_1$ 
    and with respect to each other (bottom) for each web scheme (T-Web, 
    continous lines. V-Web, dashed lines) calculated over all grid cells 
    in the Bolshoi simulation. 
    Thick central lines are the median and filled regions include
    $50\%$ of the cells in each bin. This information is necessary to 
    define void boundaries in the watershed algorithm. The vertical lines 
    show the equivalent threshold values for the T-Web and V-web in order 
    to have voids composed by cells with Fractional Anisotrop equal or 
    less than $0.95$. This threshold on the eigenvalues does not influence 
    our computations. We show it as a guide to readers familiar with the
...
\centering
  ...
  ($dV_i/dFA,\ \ i=v,s,f,k$) normalized by the total volume per FA ($dV/dFA$).
...
% CORRECTION !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
\before{In Fig. \ref{fig:L1_correlations} we show that the these two values
...
\after{In Fig. \ref{fig:L1_correlations} we show that the these two values
are indeed closely correlated.  The top panel shows the correlation between 
$\lambda_1$ and $\delta$ for all the grid cells in the simulation while the 
bottom panel shows the correlation between $\lambda_1$ and the FA. The middle 
panel shows the correlation between $\delta$ and the FA. This shows that the 
density has a larger scatter at fixed $\lambda_1$ than the FA, yet large and 
small values of the density remain still associated to low values of the FA.}
...
eigenvalue $\lambda_1$ presents, along with the density, an undesirable 
feature for a void tracer. As we move out from inner parts of voids to outer 
parts, $\lambda_1$ and the density increase monotonically, making unclear 
where to set the frontier of voids. On the other hand, the FA naturally 
indicates the real frontier as regions where anisotropic collapse begin to 
dominate, which, according to Fig. \ref{fig:FA_environment}, are associated
mainly to sheets and, to a lesser extent, to filaments.}
%!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
...
\citep{Schaap00}.
\before{However, from the analysis of our results the choice of a fixed
Cartesian mesh does not induce spurious results, at least with
the mass resolution we have in the N-body simulation. }
\after{Nevertheless, in our case the average number of particles per cell
...
% CORRECTION !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
\before{The watershed algorithm also needs a threshold value to reduce
...
\after{The watershed algorithm also needs a threshold value to reduce
spurious features and prevent void hierarchization.  
If the density field is used, a typical threshold is
$\delta = -0.8$ \citep{Platen07},  which means that, given two neighbour 
voids, if the mean value of the density across their common boundary is 
below that threshold, both voids are taken as just one void.
In our case we have to find a corresponding FA value
to define this threshold.}
%!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
...
\before{Below that scale numerical resolution effects become
  important. }
\after{For smaller interpolation/smoothing scales the shot noise in
...
\after{For larger smoothing scales ($>5\hMpc$) the density and shear fields
  tend to be Gaussian and therefore follow closely the analytical environment abundance
  predictions for Gaussian random fields \citep{Forero09,Alonso2015}. }
...
\before{We find that subcompensated voids have outflowing velocity profiles
all the way up to the effective radius where the average radial
density reaches the average value.}
\after{We find that subcompensated voids have outflowing velocity profiles
all the way up to the radius where the average radial
density reaches the average value.}
For voids with sizes $\reff <8\hMpc$ the outflow is always
...
($r/\reff =3$) and the radius of the FA  ridge ($r/\reff =1$ )
