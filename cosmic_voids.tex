...
We show that fractional anisotropy (FA) computed from the eigenvalues
of each web scheme is a good void tracer. 
Based on this result we identify voids using a watershed
transform based on the local minima of the FA field.
...
\begin{figure*}
  ...
  \caption{Visual impression of different results mentioned in this
    paper for a slice of $1\hMpc$ width in the Bolshoi simulation.  
    Fractional Anisotropy field (left); cosmic web classification
    (middle) where voids are white, sheets are light gray, filaments
    are dark gray and knots are black; and colored individual
    voids found by the a watershed algorithm on the Fractional
...
\centering
  ...
  \captionof{figure}{\small Overdensity (left) and Fractional
    ...
    Thick central lines are the median and filled regions include $50\%$ of all
    cells. 
...
In the T-Web $35\%$ of the voids are overcompensated, meaning that
they are located in denser regions with a clear delimiting ridge. 
For the V-web the subcompensated fraction increases from $65\%$ to
$85\%$ as the void size decreases, supporting the picture of smaller
spurious voids located in low density matter sheets.
...
This indicates that voids defined by FA boundaries are smaller 
than voids found by density only considerations close by a radial factor 
of $2$. 
