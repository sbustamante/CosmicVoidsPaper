...
Once we establish the FA as a conveniente tracer for voids, we proceed to
identify individual voids as basins of local minima. For this purpose we
implement a \textit{watershed transform algorithm}
\citep{Beucher79,Beucher93} which has been already implemented on void
finding schemes\citep{Platen07,Neyrinck08} defining voids as catching
basins of local minima of the density field.  
We use insted minima of the FA field to find define the voids.
...
tensor fields. 
In Section \ref{sec:experiments} we describe the setup
for the numerical experiments we perform to show the performance of
the new void finding technique. 
Our results are presented in \ref{sec:results} to finally comment and
conclude about this findings in Section \ref{sec:conclusions}. 
