...
//USAGE void_matrix_builder.out <EIG_Filename> <Lambda_th> <Out_Filename>
...
/*	fprintf(stderr, "Nx Ny Nz : %d %d %d %lld\n", n_x, n_y, n_z, n_total);
    ...
    for( i=0; i<n_total; i++ ){
	// Other type of region different to voids
	regiont_type = 0;
	// If voids regions
	if( eigen1[i] <= Lambda_th && eigen2[i] <= Lambda_th && eigen3[i] <= Lambda_th ){
	    regiont_type = 1;}
	fprintf( out, "%1d\n", regiont_type);}
